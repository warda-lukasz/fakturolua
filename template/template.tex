\documentclass[12pt]{article}
\usepackage{polski}
\usepackage[utf8]{inputenc}
\usepackage{tabularx}
\usepackage{graphicx}
\usepackage[a4paper, total={7in, 8in}]{geometry}

\setlength{\marginparwidth}{0pt}
\setlength{\parindent}{0pt}
\pagestyle{empty}

%Format
\newcommand{\nipText}{\textbf{NIP:}}
\newcommand{\regonText}{\textbf{REGON:}}
\newcommand{\bankText}{\textbf{BANK:}}
\newcommand{\accountText}{\textbf{Rachunek:}}
% Format end

\begin{document}
    \begin{footnotesize}
        \begin{flushright}
            \begin{minipage}[t]{.4\textwidth}
                Data wystawienia:~<<invoiceissueDate>> \\
                Data sprzedaży:~<<invoicesaleDate>>
            \end{minipage}
        \end{flushright}
    \end{footnotesize}

    \ \\

    \begin{small}
        \begin{minipage}[t]{.6\textwidth}
            \textbf{\emph{Sprzedawca:}} \\
            <<sellercompanyName>> \\
            <<selleraddress>> \\
            <<sellerpostalCode>>~<<sellercity>> \\ \\
            \nipText~<<sellernip>> \\
            \regonText~<<sellerregon>> \\ \\
            \bankText~<<sellerbank>> \\
            \accountText~<<selleraccount>>
        \end{minipage}
        \begin{minipage}[t]{\textwidth}
            \textbf{\emph{Nabywca:}} \\
            <<customercompanyName>> \\
            <<customeraddress>> \\
            <<customerpostalCode>>~<<customercity>> \\ \\
            \nipText~<<customernip>>
        \end{minipage}
    \end{small}

    \ \\ \ \\

    \centerline{\textbf{\LARGE{Faktura VAT nr: <<invoicetitle>>}}}

    \ \\

    \bgroup
    \def\arraystretch{2}
    \begin{table}[htb]
        \resizebox{\textwidth}{!}{% use resizebox with textwidth
            \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
                \hline
                \textbf{Lp.} & \textbf{Nazwa usługi} & \textbf{Ilość}
                & \textbf{Jm.} & \textbf{Cena netto} & \textbf{Wartość netto}
                & \textbf{Stawka Vat} & \textbf{Kwota Vat} & \textbf{Wartość Brutto} \\
                \hline
                1 & <<invoiceproduct>> & 1 & szt. & <<invoicecurrencyNet>>
                & <<invoicecurrencyNet>> & <<invoicevatPercentage>>
                & <<invoicecurrencyVat>> & <<invoicecurrencyGross>> \\
                \hline
            \end{tabular}
        }\label{tab:table}
    \end{table}
    \egroup

    \begin{small}
        \begin{minipage}[t]{.7\textwidth}
            Razem do zapłaty: <<invoicecurrencyGross>> \\
            \begin{scriptsize}
                <<invoicecurrencySpellout>>
            \end{scriptsize}
        \end{minipage}
        \begin{minipage}[t]{\textwidth}
            Sposób zapłaty: <<invoicepayment>> \\
            Termin zapłaty: <<invoicepaymentDate>>
        \end{minipage}
    \end{small}

    \ \\

    \begin{small}
        \begin{minipage}[t]{.4\textwidth}
            \centerline{
                \textbf{\emph{<<sellerfullName>>}
                }}
            \ \\
            \vspace{-4ex}
            \hline
            \vspace{.5ex}
            \scriptsize{
                \centerline{Imię i nazwisko oraz podpis osoby}
                \centerline{upoważnionej do wystawienia faktury}
            }
        \end{minipage}
        \hfill
        \begin{minipage}[t]{.4\textwidth}
            \ \\ \\
            \vspace{-4ex}
            \hline
            \vspace{.5ex}
            \scriptsize{
                \centerline{Imię i nazwisko oraz podpis osoby}
                \centerline{upoważnionej do wystawienia faktury}
            }
        \end{minipage}

        \ \\
        \ \\

        \begin{small}
            \small{\centerline{{MPK: <<invoicempkCode>>}}}
        \end{small}


    \end{small}
\end{document}

